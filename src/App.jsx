    ...
    <div style={{ minHeight:"100vh", background:"linear-gradient(135deg, #0f2240 0%, #1e3a5f 60%, #2563eb 100%)", display:"flex", alignItems:"center", justifyContent:"center", fontFamily:"'Segoe UI', system-ui, sans-serif" }}>
      <div style={{ width:"100%", maxWidth:420, padding:"0 20px" }}>
        <div style={{ textAlign:"center", marginBottom:36 }}>
          ...
          <h1 style={{ color:"white", margin:0, fontSize:24, fontWeight:700 }}>MISSION AGGLO BRIVE</h1>
          <div style={{ color:"#60a5fa", fontSize:14, marginTop:4 }}>SOCOTEC – 2026</div>
        </div>
...
function MainApp({ user, commune, onBack, onLogout }) {
  ...
  const updateMission = async (batId, missionId, field, value) => {
    ...
    try { await updateMissionDB(missionId, { [dbField]: value }); } catch(e) { console.error(e); }
  };
  ...
  const removeMission = async (batId, missionId) => {
    ...
    try { await deleteMissionDB(missionId); } catch(e) { console.error(e); }
  };
  ...
  const removeBatiment = async (batId) => {
    ...
    setConfirmDelete(null);
    try { await deleteBatimentDB(batId); } catch(e) { console.error(e); }
  };
